...
pub struct Palette {
    ...
    pub blue: Color,
    /// Terminal text selection foreground. None preserves the cell foreground.
...
fn parse_selection_fg(value: &str) -> Option<Color> {
    if value.trim().eq_ignore_ascii_case("preserve") {
        None
    } else {
        Some(crate::config::parse_color(value))
    }
}
...
mod tests {
    ...
    fn custom_selection_background_override_applies() {
        ...
        assert_eq!(palette.selection_fg, Some(Color::White));
        assert_eq!(palette.selection_bg, Color::Red);
    }

    #[test]
    fn selection_foreground_defaults_to_black() {
        assert_eq!(Palette::catppuccin().selection_fg, Some(Color::Black));
        assert_eq!(Palette::terminal().selection_fg, Some(Color::Black));
    }
